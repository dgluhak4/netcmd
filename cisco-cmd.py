...
Main function that deploys list of commands to a list of devices and parses and stores its output
    ...
    try:
        cmd_options, cmd_values = getopt.getopt(argumentList, "hpbsvd:l:c:x:r:t:q:y:", ["help","parse","bar","store","timestamp",\
            "device=","device_file_list=","command=","cmd_file_list=","repeat=","template=","query=","device_type="])
    ...
    host_ip="127.0.0.1"
    cmd="show version"
    user_device_type=default_TYPE
    invalid_option={'Device':0,'Command':0}
    ...
    for curr_opts, curr_vals in cmd_options:
        if curr_opts in ("-h", "--help"):
            ...
            if (curr_vals.isdigit()):
                ...
                repeat=eval(curr_vals[:len(curr_vals)-1])
            ...
            query_data=curr_vals
        elif curr_opts in ("-y","--device_type"):
            user_device_type=curr_vals
        elif curr_opts in ("-d","--device"):
            host_ip=curr_vals
            invalid_option['Device'] += 1     
            option['Device']=1
        elif curr_opts in ("-c","--command"):
            cmd=curr_vals
            invalid_option['Command'] +=1
            option['Command']=1
        elif curr_opts in ("-l","--device_file_list"):
            host_file_name=curr_vals
            invalid_option['Device'] += 1     
            option['Device']=0
        elif curr_opts in ("-x","--cmd_file_list"):
            command_file_name=curr_vals
            invalid_option['Command'] +=1
            option['Command']=0
        else:
            print(HELP)
            sys.exit(2)
    if invalid_option['Device'] != 1:
        print(HELP)
        sys.exit(2)
    if invalid_option['Command'] != 1:
        print(HELP)
        sys.exit(2)
    if invalid_option['Device']*invalid_option['Command'] > 0:
